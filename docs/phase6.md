...
| 检索算法 | **线性扫描 + 余弦** | HNSW/ANN 索引 | 数据规模小，线性扫描直观且正确；ANN 是第 9 期+的升级项 |
...
> 结论：对学习项目，手写词袋嵌入**唯一符合「从零吃透」目标**，并清楚暴露 RAG 的全链路；代价是语义粒度不如神经网络 embedding——这正是第 9 期「可插拔 embeddings」的升级点（把 `embed()` 换成调 API 即可，检索/存储层不动）。
...
5. 本期词袋做不到（字符无重叠）。升级：① 第 9 期把 `embed()` 换成神经网络 embedding API（同义词语义向量接近）；② 加同义词/分词词典；③ 混合 BM25 + 稠密向量。
...
- **可插拔 embedding（第 9 期）**：把 `embed()` 抽象成接口，默认手写实现，可选接入 OpenAI-compatible `/embeddings` 端点；检索/存储层零改动。这是「手写原理」与「生产语义质量」的最佳折中。
- **重排（Rerank）**：对 top-k 候选用交叉编码器精排，提升精度（RAG 经典两阶段）。
- **混合检索**：稀疏（BM25/关键词）+ 稠密（向量）融合，兼顾召回与语义。
- **MCP resources 桥接（第 9 期）**：MCP 的 `resources` 能力天然适配 RAG 语料源，可作为 `rag_search` 的外部数据源。
